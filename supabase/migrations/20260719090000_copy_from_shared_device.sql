...
begin
  ...
  with inserted as (
    ...
    select sl.id, p_target_device_id
    from public.short_links sl
    join public.short_link_device_access access
      on access.link_id=sl.id and access.device_id=source_id
    where sl.slug=any(saved_slugs)
